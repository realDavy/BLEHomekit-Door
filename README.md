...
1. 霍尔：**TI TMAG5233D1EDBVR**（立创 **C44538511** 现货），**平面内 / 侧感应**（X 轴，磁场平行于 PCB），全极推挽，40 Hz，Iq ≈ 2.7 µA。同封装的 5 Hz 版 `TMAG5233D1BDBVR` 立创常无货。不要用正面感应的 DRV5032，也不要用 A3144。
2. NMOS 反相（2N7002）：霍尔 OUT → **栅极**；**漏极 → GPIO5**（漏极不要接 3V3）；1 MΩ 上拉在 GPIO5 与 3V3 之间。GPIO5 不要直接接霍尔 OUT。
...
- 在 iPhone「家庭」里删除配件（先打开配件详情，让蓝牙连上再删）
- 开发调试可用 `idf.py erase-nvs`

然后即可重新添加。不要给 IO3 加清配对按键。
