...
固件每 2 秒打印 `pin scan IO0=… IO4=… IO5=…`（0 = 对地短路）。门磁在 **IO5**。

1. 短接门磁应出现 `GPIO5 LOW → door CLOSED`，且 **IO5=0**
2. 松开应为 `GPIO5 HIGH → door OPEN`，IO5=1
3. 若 IO5 仍是 1、IO4 变成 0：碰到的是 IO4，不是 IO5
4. 串口已 CLOSED、家庭 App 仍开着：删掉配件再添加，iPhone 靠近用蓝牙打开详情
