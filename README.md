...
| 功能 | GPIO | 说明 |
|------|------|------|
| 霍尔 / 干簧管 | **IO14** | 内部上拉。磁铁靠近（门关）= 低电平 |
| 电池分压 ADC | **IO1** | `VBAT — 1MΩ — IO1 — 1MΩ — GND`（2:1）。不接则电量显示 100% |
| 恢复出厂（配对） | **IO5** | 上电时对地保持约 1.5 秒，清除 HomeKit 配对 |

ESP32-C3 **深睡** GPIO 唤醒只有 IO0–IO5。霍尔在 **IO14**，空闲时用 **light sleep** 被 IO14 唤醒，再重启 BLE。
...
- 已配对、空闲约 **12 秒** 且没有蓝牙连接：关射频，进入 light sleep（IO14 电平变化 + 30 分钟定时唤醒）
