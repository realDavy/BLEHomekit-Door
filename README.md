...
| 功能 | GPIO | 说明 |
|------|------|------|
| 霍尔 / 干簧管 | **IO14**（测试） | 内部上拉。磁铁靠近（门关）= 低电平。深睡 GPIO 唤醒只有 IO0–5，测试期间不深睡 |
| 电池分压 ADC | **IO1** | `VBAT — 1MΩ — IO1 — 1MΩ — GND`（2:1）。不接则电量显示 100% |
| 恢复出厂（配对） | **IO3** | 上电时对地保持约 1.5 秒，清除 HomeKit 配对 |

当前固件把门磁临时接到 **IO14** 方便测试。C3 深睡 GPIO 唤醒只有 IO0–IO5，所以这段测试期间不会进深睡，串口可以一直看着 `GPIO14 LOW → door CLOSED`。
...
- 已配对、空闲约 **12 秒** 且没有蓝牙连接：关射频，进入深睡（门磁在 IO0–5 时：电平变化 + 30 分钟定时唤醒）。门磁在 IO14 测试时不深睡
...
固件每 2 秒打印 `pin scan … IO14=…`（0 = 对地短路）。当前测试脚是 **IO14**。

1. 短接 IO14 应出现 `GPIO14 LOW → door CLOSED`，且 **IO14=0**
2. 松开应为 `GPIO14 HIGH → door OPEN`，IO14=1
3. 串口已 CLOSED、家庭 App 仍开着：删掉配件再添加，iPhone 靠近用蓝牙打开详情
